...
async def get_vpn(callback: CallbackQuery):
    ...
    try:
        result = await asyncio.to_thread(
            provision_user, server, client_uuid, f"tg-{user_id}", SSH_TIMEOUT
        )
    except Exception:
        logging.exception("Provisioning failed")
        await callback.message.answer(
            "❌ Не удалось создать конфигурацию. "
            "VPS ещё не подключён или недоступен."
        )
        return
    ...
    await callback.message.answer(
        "✅ <b>VPN готов</b>\n\n"
        "Импортируйте ссылку в V2RayTun:\n\n"
...
async def addvps(message: Message):
    ...
    server_id = db.add_server(country, name, host, ssh_user, ssh_key)
